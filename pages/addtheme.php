...
echo "<a class='person' href='pages/viewuser.php?id=".$array0['id']."'><button type='submit'>".$array0['login']."</button></a>,";
    ...
    <div class="nav-wrapper xxx">
      <div class="col s12">
        <a href="javascript:history.back()" class="breadcrumb">Назад</a>
		<a href="#!" class="breadcrumb">Добавить тему</a>
      </div>
    </div>
  </nav>
...
<!--**** testreg.php - это адрес обработчика. То есть, после нажатия на кнопку "Войти", данные из полей отправятся на страничку testreg.php методом "post" ***** -->
  <p>
    ...
    <input required  name="name" type="text" size="15" maxlength="100">
  </p>
    <p>
    <label>Раздел:<br></label>
	ВЫБРАТЬ ЗДЕСЬ .
	<?php 
	...
	do{
	?>
	"<?php echo $razdel['name'];?>"/
	<?php
		}while($razdel = mysql_fetch_array($query1));
	?>
    <input required  name="razdel" type="text" size="15" maxlength="100">
  </p>
<!--**** В текстовое поле (name="login" type="text") пользователь вводит свой логин ***** -->  

  	<input required style="display:none;" name="author" id="author" value="<?php echo $_SESSION['login'];?>">
    <label>Текст:<br></label>
    <textarea name="maintext" type="text" size="160" maxlength="30000"></textarea>
  </p>
