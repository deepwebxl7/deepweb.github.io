...
if(!empty($_GET['id']))
{
$themesanalize = mysql_query("SELECT * FROM themes WHERE `id` = '{$_GET['id']}'");
$theme = mysql_fetch_array($themesanalize);
	$authort = $theme['author'];
...
echo "<a class='person' href='pages/viewuser.php?id=".$array0['id']."'><button type='submit'>".$array0['login']."</button></a>,";
    ...
    <div class="nav-wrapper xxx">
      <div class="col s12">
		...
		<a href="#!" class="breadcrumb">Просмотр темы</a>
      </div>
    </div>
  </nav>
  <div class="themefull">
<span style="font-size:20pt;"><?php echo $theme['name'];?></span><br>
<span><?php
		  if($array4['progres'] == '5'){
			  $adminnenadoc = "none";
		  }else{
		  ...
		  if($theme['author'] == $_SESSION['login']){$ads = 'true';}
		  else{$ads = $ads;}
		  ?>
...
<button type="submit">Delete</button>
		  ...
		  <span style="display:<?php echo $ad;?>;"><a style="display:<?php echo $adminnenadoc;?>;" href="ch_themestt.php?id=<?php echo $array3['id'];?>">
<button type="submit">Change Status</button>
</a></span></div></td>
<?php
echo "<img src='".$array4['photo']."' class='circle photoprofile'><a class='person' href='viewuser.php?id=".$array4['id']."'><button type='submit'>".$theme['author']."</button></a><span style='color:".$colll.";'>".$array4['online']."</span>";
?></span><br>
<span class="right" style="font-size:14pt;"><?php echo $theme['datetime'];?></span><br>
<span style="font-size:14pt;"><?php
echo "<span class='provereno' style='display:".$theme['status'].";'><i class='tiny material-icons'>check_circle</i> Проверено</span>";
?></span><br>
<p><?php echo $theme['maintext'];?></p>
</div>
      </div>
